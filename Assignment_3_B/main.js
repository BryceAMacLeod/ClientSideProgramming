...
(() => {
    ...
    function getTotalEpisodesInYear(data, year) {
        return data._embedded.episodes
        .filter((episode) => episode.airdate.includes(year))
        .length;
    }
    //4 - Create a function called getFemaleCastMembers() that returns an array of the names of the female cast members.
    function getFemaleCastMembers(data) {
        return data._embedded.cast
        .filter((member) => member.person.gender === "Female")
        .reduce((femaleCast, member) => {
            femaleCast.push(member.person.name);
            return femaleCast;
        },[]);
    }
    //5 - Create a function called getEpisodeTitles() which returns a list of episode
    ...
    function getCastMembersOver55(data) { 
        targetYear = new Date().getFullYear() - 55;
    ...
    function capitalizeTheFriends(data) {
        ...
        .map((episode) => {
            ...
            episode.name = episode.name.replace(/Monica/g, "MONICA");
            episode.name = episode.name.replace(/Rachel/g, "RACHEL");
            episode.name = episode.name.replace(/Phoebe/g, "PHOEBE");
            episode.name = episode.name.replace(/Ross/g, "ROSS");
            episode.summary = episode.summary.replace(/Joey/g, "JOEY");
            episode.summary = episode.summary.replace(/Chandler/g, "CHANDLER");
            episode.summary = episode.summary.replace(/Monica/g, "MONICA");
